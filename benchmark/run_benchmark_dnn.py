#!/usr/bin/env python3
# ============================================================
# run_benchmark_dnn.py
# Benchmark runner untuk model DNN dan CNN (TFLite Micro)
# pada ESP32, Arduino Uno, dan Arduino Nano.
#
# Ekstensi dari run_benchmark.py (DT/RF/KNN) — format output
# dan metodologi IDENTIK untuk komparasi yang fair.
#
# Usage:
#   python run_benchmark_dnn.py --platform ESP32 --model dnn \
#     --protocol balanced --seed 42 --port COM4 --samples 1000
#
# Supports combinations:
#   ESP32:  DNN/CNN × (balanced 42/43/44 + natural 99) = 8
#   Uno:    DNN/CNN × (balanced 42/43/44 + natural 99) = 8
#   Nano:   DNN/CNN × (balanced 42/43/44 + natural 99) = 8
#   Total: 24 runs (+ 28 existing DT/RF/KNN = 52 total)
#
# Output:
...
#
# Catatan:
#   - ESP32: firmware ids_esp32_dnn.ino (DNN + CNN sekaligus)
#   - Uno/Nano: firmware ids_arduino_{uno,nano}_dnn.ino
#     (harus re-upload antara DNN dan CNN, pilih via #define)
# ============================================================
...
# ============================================================
# DATASET LOADING (reuse dari run_benchmark.py)
# ============================================================

def load_benchmark_dataset(protocol, seed, samples, data_dir=None):
    """Load pre-generated benchmark dataset."""
    if data_dir is None:
        data_dir = os.path.dirname(os.path.abspath(__file__))
    
    x_file = f"bench_{protocol}_X_raw_{samples}_seed{seed}.csv"
    y_file = f"bench_{protocol}_y_{samples}_seed{seed}.csv"
    
    x_path = os.path.join(data_dir, x_file)
    y_path = os.path.join(data_dir, y_file)
    
    if not os.path.exists(x_path):
        sys.exit(f"ERROR: Dataset not found: {x_path}\n"
                 f"Run prepare_benchmark_sets.py first!")
    
    X = pd.read_csv(x_path).values
    y = pd.read_csv(y_path, header=None).values.ravel()
    
    assert len(X) == samples, f"Expected {samples}, got {len(X)}"
    assert len(y) == samples, f"Expected {samples} labels, got {len(y)}"
...
# ============================================================
# PARSING (sama format dgn DT/RF/KNN)
# ============================================================

def parse_response_esp32(responses):
    """Parse ESP32 DNN benchmark response.
...
def parse_response_avr(responses):
    """Parse Arduino Uno/Nano DNN response.
...
def run_single_benchmark(serial_conn, X_raw, y, model_name,
    ...
    """Run full benchmark — identical logic to run_benchmark.py."""
    ...
    for i in range(n_samples):
        ...
        # DNN/CNN inference pada mikrokontroler mungkin sedikit lebih lambat
        if not responses:
            time.sleep(0.15)
        ...
        if not responses:
            time.sleep(0.3)
            responses = serial_conn._read_all(timeout_extra=0.15)
        ...
        if pred is not None:
            ...
            per_sample.append({
                'idx': i, 'pred': pred, 'true': true_label,
                'correct': pred == true_label, 'inference_us': inf_time
            })
        else:
            missed += 1
            per_sample.append({
                'idx': i, 'pred': None, 'true': true_label,
                'correct': None, 'inference_us': None
            })
            if missed <= 5:
                print(f"  WARNING: No response for sample {i}")
            elif missed == 6:
                print(f"  WARNING: Suppressing further warnings...")
...
def qc_check(result):
    ...
    if parsed != n:
        issues.append(f"WARN: Only {parsed}/{n} parsed ({result['n_missed']} missed)")
    
    cm_sum = result['tp'] + result['fp'] + result['tn'] + result['fn']
    if cm_sum != parsed:
        issues.append(f"FAIL: CM sum={cm_sum} != parsed={parsed}")
    
    if result['protocol'] == 'balanced':
        n_atk = result['tp'] + result['fn']
        n_nor = result['tn'] + result['fp']
        if n_atk != n // 2 or n_nor != n // 2:
            issues.append(f"WARN: Balanced off: N={n_nor}, A={n_atk}")
...
def print_result_summary(result):
    print(f"\n{'=' * 60}")
    print(f"  BENCHMARK RESULT — {result['model']} ({result.get('framework', 'TFLite')})")
    print(f"{'=' * 60}")
    ...
    print(f"  Avg Inf.  : {result['avg_inference_us']:.1f} μs")
    print(f"  Std Inf.  : {result['std_inference_us']:.1f} μs")
    print(f"  Min Inf.  : {result['min_inference_us']:.1f} μs")
    print(f"  Max Inf.  : {result['max_inference_us']:.1f} μs")
...
def main():
    parser = argparse.ArgumentParser(
        description='IDS DNN/CNN Benchmark Runner',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Single run
  python run_benchmark_dnn.py --platform ESP32 --model dnn --protocol balanced --seed 42

  # All DNN datasets on ESP32 (4 runs)
  python run_benchmark_dnn.py --platform ESP32 --model dnn --protocol all

  # All DNN+CNN on ESP32 (8 runs)
  python run_benchmark_dnn.py --platform ESP32 --model all --protocol all

  # DNN on Arduino Uno (4 runs)
  python run_benchmark_dnn.py --platform ArduinoUno --model dnn --protocol all --port COM3
        """)
    ...
    print(f"\n{'=' * 60}")
    print(f"  IDS DNN/CNN BENCHMARK PLAN")
    print(f"{'=' * 60}")
    ...
    print(f"  Framework: TensorFlow Lite Micro (INT8)")
    print(f"{'=' * 60}")
    
    if platform != 'ESP32':
        if len(models) > 1:
            print(f"\n  NOTE: For {platform}, re-upload firmware between DNN and CNN")
            
            for model in models:
                print(f"\n{'*' * 60}")
                print(f"  NOW RUNNING: {model.upper()} on {platform}")
                print(f"  Make sure firmware is uploaded with "
                      f"USE_{'DNN' if model == 'dnn' else 'CNN'}")
                print(f"{'*' * 60}")
                input(f"  Press ENTER when {model.upper()} firmware is ready...")
    ...
    print(f"\n\n{'=' * 60}")
    print(f"  ALL DNN/CNN RUNS COMPLETE ({total_runs} runs)")
    print(f"  Results saved to: {output_dir}")
    print(f"{'=' * 60}")
    
    for f in sorted(Path(output_dir).glob("*.json")):
        if 'DNN' in f.name or 'CNN' in f.name:
            print(f"  {f.name}")
