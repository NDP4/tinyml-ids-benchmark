// ============================================================
// IDS (Intrusion Detection System) — Arduino Uno + DNN
...
// Pilih model (hanya DNN untuk Uno — CNN mungkin terlalu besar)
#define USE_DNN
// #define USE_CNN  // Uncomment jika ukuran model CNN cukup kecil
...
#define LED_ALERT 13          // Built-in LED

// TFLite Micro arena — HARUS kecil untuk AVR (2KB RAM total!)
// DNN kecil: ~2-3KB arena
constexpr int TENSOR_ARENA_SIZE = 3 * 1024;  // 3 KB (sesuaikan)
...
int predict(float* features) {
    ...
    int num_input = model_input->bytes;  // Jumlah elemen input
    ...
    if (Serial.available() > 0) {
        ...
        if (input.length() > 0) {
            if (parseFeatures(input, raw_features)) {
                ...
                // Format output kompatibel dgn run_benchmark.py (AVR parser)
